...
import cn.sspku.data.vo.PermissionDTO;
import cn.sspku.data.vo.RoleDTO;
...
public class SecurityUtil {
    ...
    @Autowired
    private sspkuLoginProperties tokenProperties;
    ...
    public String getToken(String username, Boolean saveLogin){
        if(sspkuNullUtils.isNull(username)){
            throw new sspkuException("username不能为空");
        }
        boolean saved = false;
        ...
        if(tokenProperties.getSaveRoleFlag()){
            for(PermissionDTO p : selectUser.getPermissions()){
                if(!sspkuNullUtils.isNull(p.getTitle()) && !sspkuNullUtils.isNull(p.getPath())) {
        ...
        if(tokenProperties.getSsoFlag()) {
            String oldToken = redisTemplate.opsForValue().get(sspkuLoginProperties.USER_TOKEN_PRE + selectUser.getUsername());
            if (StrUtil.isNotBlank(oldToken)) {
                redisTemplate.delete(sspkuLoginProperties.HTTP_TOKEN_PRE + oldToken);
            }
        }
        // 保存至Redis备查
        if(saved){
            redisTemplate.opsForValue().set(sspkuLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
            redisTemplate.opsForValue().set(sspkuLoginProperties.HTTP_TOKEN_PRE + ansUserToken, new Gson().toJson(tokenUser), tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
        }else{
            redisTemplate.opsForValue().set(sspkuLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
            redisTemplate.opsForValue().set(sspkuLoginProperties.HTTP_TOKEN_PRE + ansUserToken, new Gson().toJson(tokenUser), tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
        }
        return ansUserToken;
    }
