package cn.sspku.basics.code.vue;
...
import cn.sspku.basics.code.bean.Field;
...
@RestController
@Api(tags = "Vue代码生成")
@RequestMapping(value = "/sspku/generate")
public class sspkuVueGenerator {
    ...
    public String generate(String template, boolean api, String vueName, Integer rowNum, List<Field> fields) throws IOException {
        ...
        if(rowNum == 1){
            ...
            // 不满足的情况
            throw new sspkuException("一行的输入栏只能为1到4列");
        }
    ...
    public static String name(String name, boolean isFirstUpper){
        ...
        if(StrUtil.isBlank(name)){
            throw new sspkuException("name不能为空");
        }
