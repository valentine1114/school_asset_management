...
public class RedisRaterLimiter {
    ...
    private static final String LIMIT_MAX_PRE = "sspku_LIMIT_MAX:";

    private static final String LIMIT_NOW_PRE = "sspku_LIMIT_NOW:";
    ...
    public boolean getLimitFlag(@ApiParam(name = "IP地址等唯一标识") String point, @ApiParam(name = "限流次数") Integer limit, @ApiParam(name = "限流单位时间") Long timeout) {
        ...
        try {
            ...
            if(sspkuNullUtils.isNull(maxCount)){
                // 重置次数
                ...
                return true;
            } else if (!sspkuNullUtils.isNull(maxCount) && !sspkuNullUtils.isNull(currCount)){
