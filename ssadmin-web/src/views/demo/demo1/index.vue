        ...
        <Form ref="searchForm" :model="searchForm" inline :label-width="0">
          ...
          >
            <Select
              ...
              style="width: 100px"
            >
              <Option value="inventory">盘点表</Option>
              <Option value="idle">闲置表</Option>
              <Option value="scrap">报废表</Option>
              <Option value="disposal">处置表</Option>
            </Select>
            <!-- 资产名称子级盒子 -->
            ...
            <Form-item label="" prop="warehouse">
              <Select v-model="searchForm.warehouse" placeholder="请选择仓库">
                <Option
            ...
            <Form-item label="" prop="giveDepart">
              <Select v-model="searchForm.giveDepart" placeholder="请选择部门">
                <Option
            ...
            <!-- 右侧俩按钮栏 -->
            <Form-item style="position: fixed; right: 30px; top: 110px">
              <!-- 列筛选按钮 -->
              <Poptip
                trigger="hover"
...
export default {
      ...
      columns: [
        ...
        // },
        {
          ...
          render: (h, params) => {
            ...
            // if (params.row.auditRetireFlag == 1) {
            //   return h("div", [
            //     h(
            //       "span",
            //       {
            ...
            if (params.row.status !== 2) {
              ...
              return h("div", [
                h(
                  ...
                  "[" + params.row.giveDepart + "] " + params.row.user
                ),
              ]);
            }
          },
  ...
  methods: {
    ...
    fetchWarehouses() {
      ...
      getAdminWareList({})
        .then((res) => {
          this.loading = false;
          if (res.success) {
            this.warehouses = res.result.map((ware) => ({
              value: ware.name, // 确保这是正确的字段名
              label: ware.name, // 确保这是正确的字段名
            }));
          } else {
    ...
    fetchDepartments() {
      ...
      getAdminDepartmentList({})
        .then((res) => {
          ...
          if (res.success) {
            console.log(res);
            this.departments = res.result.map((depart) => ({
              value: depart.title, // 确保这是正确的字段名
              label: depart.title, // 确保这是正确的字段名
            }));
          } else {
            console.error("Unexpected response structure:", res);
          }
    ...
    loadTableData() {
      ...
      const selectedType = this.selectedTable;

      // 设置请求参数
      const params = {
        ...this.searchForm, // 包含原有的搜索参数
        // 这里可以添加其他查询参数，如分页信息
        tableType: selectedType, // 这是你选中的表类型
      };
      // 开始加载数据
      this.loading = true;

      // 发送请求到后端
      getAdminAssetsListSearch(params)
        .then((res) => {
          this.loading = false;
          if (res.success) {
    ...
    // 搜索
    handleSearch() {
      console.log("Search waretriggered", this.searchForm.warehouse);
      console.log("Search departtriggered", this.searchForm.giveDepart);
    ...
    handleReset() {
      ...
      this.searchForm.pageNumber = 1;
      this.searchForm.pageSize = 10;
      this.selectedTable = null; // 重置选中的表格类型
      this.getDataList();
    },
    // 切换查询排序规则
    changeSort(e) {
    ...
    getDataList() {
      ...
      this.loading = true;
      getAdminAssetsListSearch(params).then((res) => {
        this.loading = false;
        if (res.success) {
          this.data = res.result.records;
          console.log(this.data);
  ...
  mounted() {
    this.init();
    this.fetchWarehouses(); // 当组件挂载时获取仓库列表
