...
<script>
import {
  getAdminAssetsRegisterList,
  deleteAdminAssetsRegister,
  auditData,
  returnData,
} from "./api.js";
...
export default {
      ...
      columns: [
        ...
        // },
        {
          ...
          render: (h, params) => {
            ...
            } else if (params.row.auditFlag == 1) {
              return h("div", [
                h(
                  "span",
                  {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      getAdminAssetsRegisterList(this.searchForm).then((res) => {
        this.loading = false;

        if (res.success) {
          // 过滤掉submitFlag为0的数据项
          const filteredData = res.result.records.filter(
            (item) => item.submitFlag !== 0
          );
