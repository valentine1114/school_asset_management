      ...
      <Row v-show="openSearch" @keydown.enter.native="handleSearch">
        <Form ref="searchForm" :model="searchForm" inline :label-width="0">
          ...
          >
            ...
            <Form-item label="" prop="repairName">
              <Input
                type="text"
            ...
            <Form-item style="margin-left: 10px" class="br">
              ...
              >
              <!-- 筛选按钮 -->
              <Button @click="toggleRepairFilter">
                {{ repairFilter ? "显示所有" : "只显示未维修" }}
              </Button>
            </Form-item>
...
export default {
  ...
  data() {
    return {
      repairFilter: false, // 控制筛选状态，默认为false
      tableHeight: 0,
      selected: [
        "选择",
        "序号",
          ...
          render: (h, params) => {
            return h("div", [
              h(
                ...
                {
                  style: {
                    color: params.row.retireFlag == 2 ? "#ff0000" : "#000000",
                  },
                },
                params.row.retireFlag == 2 ? "[ 已报废 ]" : "[ 未报废 ]"
              ),
            ]);
          },
        },
        {
          ...
          render: (h, params) => {
            ...
            // 当 auditFlag 为 1 时，显示已被操作文本
              ...
              params.row.auditRetireFlag === 1 ||
              params.row.auditRetireFlag === 2
            ) {
              return h("div", [
                h(
                  "span",
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      getAdminAssets(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
          ...
          // // this.data = res.result.records;
          // this.data = res.result.records.filter(
          //   (item) => item.repairFlag === 1
          // );
          // console.log(this.data);
          // this.total = res.result.total;
          ...
          //   this.data = records.filter((item) => item.repairFlag === 1);
          // } else {
          //   this.data = records;
          // }
          // this.total = res.result.total;
