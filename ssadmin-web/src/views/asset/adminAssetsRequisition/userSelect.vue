<template>
  <div>
    ...
    >
      <car>
        <Row>
          ...
          >
            ...
            <Form-item label="工号">
              <Input
                type="text"
                v-model="huasearchForm.jobnumber"
                clearable
                placeholder="请输入工号"
                style="width: 200px"
              />
            ...
            <Form-item>
              <Button @click="huahandleSearch" type="primary" icon="ios-search"
                >搜索</Button
              >
        ...
        <Button @click="huashowHuaSelect" type="primary">确认选择</Button>
  ...
  <script>
import { getRosterUserByPage } from "./api.js";
...
export default {
  ...
  methods: {
    ...
    // },
    huashowHuaSelect() {
      if (this.selectHuaList.length === 0) {
        // 没有选择任何员工
      ...
      // 可以在这里处理选择的员工
      // 例如，如果你需要选择单个员工，则可以获取第一个选中的员工
      const selectedEmployee = this.selectHuaList[0];

      // 然后可以使用这个选中的员工进行后续操作
      this.$emit("submited", selectedEmployee);

      // 如果你需要处理所有选中的员工，则可以遍历 selectHuaList
      // this.selectHuaList.forEach(employee => {
      //     // 处理每个选中的员工
      // });
    },
